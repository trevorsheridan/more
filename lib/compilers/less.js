(function() {
  ...
  exports.Less = Less = (function() {
    ...
    Less.prototype.parse = function() {
      ...
      if ((typeof this.f !== "string") || (this.f === null)) {
        throw new Error("In order to parse, filename \"@f\" must be defined and of type `string`");
      }
      try {
        (new less.Parser({
          paths: (function() {
            var child, dirs, _i, _len, _ref;
            dirs = ['.', './' + path.relative(process.cwd(), path.dirname(_this.f))];
            _ref = _this.childDirs(path.dirname(_this.f));
